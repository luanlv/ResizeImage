...
(function(window, BigPipe) {
  ...
  var view = function(data){
     return m("li.ra-5", [
       m(".img", [
         m("img[alt=''][src='/image/thumb/1/" + data.image +"']")
       ]),
       m(".codeP", [
         m("a.pjContainer[href='/product/v/" + data.code + "']", [m('b', data.code)]),
         m(".stock", data.stock)
       ]),
       m(".nameP", 	data.name),
       m(".priceP", [
         "\n					Giá bán lẻ:",
         m(".price", [m("b", data.price+" VNĐ")])
       ])
     ])
  }
  ...
  BigPipe.renderPagelet = function(id, json) {
    ...
    if (domElement) {
      ...
      m.render(domElement, m('ul.gallery'));
      var domElementGallery = document.getElementById(id).firstElementChild;
      m.render( domElementGallery,
        m( 'div', { config : appendDOM() } )
      );

    } else {
      alert("ERROR: cannot render pagelet because DOM node with id " + id + " does not exist");
    }
  };
